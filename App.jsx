...
const USER_ID = import.meta.env.VITE_USER_ID || "user_001";
...
const api = {
  async chat(message, history) {
    const r = await fetch(`${API_URL}/api/chat`, { method:"POST", headers:h, body:JSON.stringify({userId:USER_ID, message, history}) });
    if (!r.ok) throw new Error((await r.json()).error || "API error");
    return r.json();
  },
  async getMemories() {
    const r = await fetch(`${API_URL}/api/memories/${USER_ID}`, { headers:h });
    return (await r.json()).memories;
  },
  async deleteMemory(id) { await fetch(`${API_URL}/api/memories/${USER_ID}/${id}`, { method:"DELETE", headers:h }); },
  async editMemory(id, content) { await fetch(`${API_URL}/api/memories/${USER_ID}/${id}`, { method:"PATCH", headers:h, body:JSON.stringify({content}) }); },
  async clearAll() { await fetch(`${API_URL}/api/memories/${USER_ID}`, { method:"DELETE", headers:h }); }
};
...
function Bubble({ msg }) {
  ...
  return (
    <div style={{display:"flex",gap:10,alignItems:"flex-end",flexDirection:u?"row-reverse":"row",marginBottom:14}}>
      <div style={{width:28,height:28,borderRadius:"50%",flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,fontWeight:600,background:u?C.teal:C.orange,color:C.white}}>
        {u?"AY":"G"}
      </div>
  ...
  return (
    <div style={{background:C.white,border:`1px solid ${C.border}`,borderRadius:10,padding:"9px 11px",marginBottom:7}}>
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginTop:6}}>
        <span style={{fontSize:10,padding:"2px 7px",borderRadius:99,fontWeight:500,...cs}}>{m.category}</span>
        <div style={{display:"flex",gap:4}}>
          <button onClick={()=>onEdit(m.id,m.content)} style={{width:22,height:22,borderRadius:6,border:`1px solid ${C.border}`,background:C.white,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",color:C.hint}}>
            <i className="ti ti-edit" style={{fontSize:12}} aria-hidden="true"/>
          </button>
          <button onClick={()=>onDelete(m.id)} style={{width:22,height:22,borderRadius:6,border:`1px solid ${C.border}`,background:C.white,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",color:C.hint}}>
...
export default function App() {
  ...
  return (
    <>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Mulish:wght@400;500;600;700&display=swap');*{box-sizing:border-box;margin:0;padding:0}body{font-family:'Mulish',sans-serif}::-webkit-scrollbar{width:4px}::-webkit-scrollbar-thumb{background:#ddd;border-radius:99px}@keyframes dp{0%,80%,100%{transform:scale(1);opacity:.5}40%{transform:scale(1.3);opacity:1}}`}</style>
      ...
      <div style={{display:"flex",height:"100vh",fontFamily:"'Mulish',sans-serif",background:C.white,overflow:"hidden"}}>
          ...
          <div style={{flex:1,padding:"10px 0"}}>
            {[["message-2","AI assistant",true],["chart-bar","Analytics",false],["users","Retention",false],["coin","Cost insights",false]].map(([icon,label,active])=>(
              <div key={label} style={{display:"flex",alignItems:"center",gap:8,padding:"8px 16px",fontSize:12.5,color:active?C.orange:"rgba(255,255,255,0.6)",background:active?"rgba(203,60,4,0.18)":"transparent",borderRight:active?`2px solid ${C.orange}`:"2px solid transparent",cursor:"pointer"}}>
                <i className={`ti ti-${icon}`} aria-hidden="true" style={{fontSize:14}}/>
                {label}
              </div>
            ))}
            ...
            <div onClick={()=>setShowMem(p=>!p)} style={{display:"flex",alignItems:"center",gap:8,padding:"8px 16px",fontSize:12.5,color:showMem?C.orange:"rgba(255,255,255,0.6)",background:showMem?"rgba(203,60,4,0.18)":"transparent",borderRight:showMem?`2px solid ${C.orange}`:"2px solid transparent",cursor:"pointer"}}>
              <i className="ti ti-brain" aria-hidden="true" style={{fontSize:14}}/>
              Your memories
        ...
        <div style={{flex:1,display:"flex",flexDirection:"column",minWidth:0}}>
          <div style={{height:50,background:C.white,borderBottom:`1px solid ${C.border}`,display:"flex",alignItems:"center",padding:"0 18px",gap:10,flexShrink:0}}>
            ...
            <button onClick={()=>setMessages([{id:uid(),role:"assistant",content:"Chat cleared. Memories intact.",ts:Date.now()}])} style={{height:28,padding:"0 10px",borderRadius:8,border:`1px solid ${C.border}`,background:C.white,fontSize:11,cursor:"pointer",color:C.muted,fontFamily:"'Mulish',sans-serif"}}>New chat</button>
            <button onClick={()=>setShowMem(p=>!p)} style={{height:28,padding:"0 10px",borderRadius:8,border:`1px solid ${showMem?C.orange:C.border}`,background:showMem?C.orange:C.white,fontSize:11,cursor:"pointer",color:showMem?C.white:C.muted,fontFamily:"'Mulish',sans-serif"}}>
            ...
            {messages.length===1&&(
              <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:14}}>
                {["My team has high churn — where do I start?","We overspent on vendors last quarter","I prefer concise data-driven responses"].map(h=>(
                  <button key={h} onClick={()=>{setInput(h);inputRef.current?.focus();}} style={{fontSize:11,padding:"5px 10px",borderRadius:99,border:`1px solid ${C.border}`,background:C.white,cursor:"pointer",color:C.muted,fontFamily:"'Mulish',sans-serif"}}>{h}</button>
                ))}
              </div>
            )}
        ...
        {showMem&&(
          <div style={{width:250,borderLeft:`1px solid ${C.border}`,background:"#fdfcfc",display:"flex",flexDirection:"column",flexShrink:0}}>
            ...
            <div style={{padding:"7px 12px",borderBottom:`1px solid ${C.border}`}}>
              <input value={memSearch} onChange={e=>setMemSearch(e.target.value)} placeholder="Search…" style={{width:"100%",height:28,border:`1px solid ${C.border}`,borderRadius:8,padding:"0 10px",fontSize:12,fontFamily:"'Mulish',sans-serif",outline:"none",color:C.dark}}/>
            </div>
            <div style={{padding:"5px 12px 7px",display:"flex",gap:4,flexWrap:"wrap",borderBottom:`1px solid ${C.border}`}}>
              {["all","preference","personal","fact","goal"].map(f=>(
                <button key={f} onClick={()=>setMemFilter(f)} style={{fontSize:10,padding:"2px 7px",borderRadius:99,cursor:"pointer",fontFamily:"'Mulish',sans-serif",background:memFilter===f?C.teal:C.white,border:`1px solid ${memFilter===f?C.teal:C.border}`,color:memFilter===f?C.white:"#666"}}>
                  {f}
                </button>
              ))}
            </div>
            <div style={{flex:1,overflowY:"auto",padding:"7px 12px"}}>
              {filtered.length===0
                ...
                : filtered.map(m=><MemCard key={m.id} m={m} onDelete={id=>{api.deleteMemory(id);setMemories(p=>p.filter(x=>x.id!==id));}} onEdit={(id,old)=>{const v=window.prompt("Edit:",old);if(v?.trim()){api.editMemory(id,v.trim());setMemories(p=>p.map(x=>x.id===id?{...x,content:v.trim()}:x));}}}/>)
              }
            </div>
            {memories.length>0&&(
              <div style={{padding:"9px 12px",borderTop:`1px solid ${C.border}`}}>
                <button onClick={()=>{if(window.confirm("Delete all?"))api.clearAll().then(()=>setMemories([]));}} style={{width:"100%",height:28,borderRadius:8,border:"1px solid #f5c8b5",background:C.white,fontSize:11,cursor:"pointer",color:C.orange,fontFamily:"'Mulish',sans-serif"}}>
                  Clear all memories
                </button>
              </div>
            )}
